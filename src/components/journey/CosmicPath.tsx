...
function TimeStrand({ curve, offset, opacity, width, speed }: { curve: THREE.CatmullRomCurve3, offset: number, opacity: number, width: number, speed: number }) {
  ...
  const strandCurve = useMemo(() => {
      ...
      return new THREE.TubeGeometry(curve, 600, width, 8, false);
  }, [curve, width]);

  useFrame((state) => {
      // Animate texture or color shift if we had textures. 
      // For now, let's just pulse opacity slightly
      if (materialRef.current) {
          materialRef.current.opacity = opacity + Math.sin(state.clock.elapsedTime * speed + offset) * 0.1;
      }
  });
...
export function CosmicPath({ curve }: { curve: THREE.CatmullRomCurve3 }) {
  return (
    <group>
        {/* Core hot beam */}
        <TimeStrand curve={curve} width={0.15} opacity={0.9} offset={0} speed={2} />
        
        {/* Outer glow 1 */}
        <TimeStrand curve={curve} width={0.4} opacity={0.4} offset={1} speed={1.5} />
        
        {/* Outer glow 2 */}
        <TimeStrand curve={curve} width={0.8} opacity={0.15} offset={2} speed={1} />
        
        {/* A second color strand for complexity (Blue/Cyan mixed with Red/Pink) */}
        <mesh>
             <tubeGeometry args={[curve, 600, 0.3, 8, false]} />
             <meshBasicMaterial color="#3b82f6" transparent opacity={0.1} blending={THREE.AdditiveBlending} side={THREE.BackSide} depthWrite={false} />
        </mesh>
    </group>
  );
}
...
export function CosmicPathGlow({ curve }: { curve: THREE.CatmullRomCurve3 }) {
    ...
    <mesh geometry={geometry}>
        <meshBasicMaterial color="#8b5cf6" transparent opacity={0.03} side={THREE.BackSide} blending={THREE.AdditiveBlending} depthWrite={false} />
    </mesh>
    )
}
